...
- **Link a statement to evidence.** A meeting move: pick something someone said (an alibi, a sighting said aloud, a public excuse) and one thing you know (a record, a sighting, hearsay, an expert's word, another alibi, their earlier words) — or two independent pieces. Judged from content only (places, people, times, the excuse's method), so every logically equivalent piece of evidence works. Results: contradiction / narrows / supports / hearsay only / same source / already explained / unclear / irrelevant. A link that holds makes the named person answer (Nulls by style: redirect, partial concession, credibility attack, silence; innocents explain a harmless reason or hold with their companions) and the room react; an irrelevant one costs a little trust.
- **The room weighs what was said aloud.** Board alibi conflicts count for NPCs only once raised (by the explorer or occasionally a careful crew member); a public confession takes a conflict off the table. Unasked keepers rarely volunteer their records; a lone private glimpse is voted on but not accused with until the explorer heard it or a second source agrees. Evidence curve widened so one trace rarely names one person. A Null picks one scapegoat per Day from public facts.
- **Meetings**: 3 strong moves + 3 clarifying questions per Day (was 1 + 3). Clarifying questions follow the point on the table (who opened a record, what exactly was seen, time check, ask the named companions, where hearsay came from, whether a secret is related). The board shows each person's state in words; the meeting ends with a plain summary; final words answer what was actually raised; the ballot asks for one reason from what you know (or instinct) and keeps it with the Day.
- **Defeat and rewind**: a lost Stage can be rewound once to that Day's morning — same reconstruction, same truth, only the explorer remembers (notes + "지난번에 들은 말" leads, no knowledge). "Back to the previous Stage" is offered on a loss. Deep has no rewind.
- **Walking rebuilt**: whole-frame walking only (no upper-body band motion while moving), a 1px rise on passing frames, per-sheet stride measured from the side-view steps (56–112 px), torso-anchored frames (axis jitter ≤ 3 px), 180° turns pass through a side/front frame while standing or walking, reduced motion still walks. New gate `motion_100_tests.gd` (14 characters × 4 directions: real leg alternation, one floor line, height, axis, stride, transitions).
- **Explorer selection**: main illustration only — pixel preview removed; cards are crops of the same drawing.
- **Explorer relationships**: first-contact tone is remembered for the Stage (bounded ±0.02 trust) and returns in that person's first line the next Day.
- **Tasks rebuilt** on a shared ship-equipment housing (reset, leave = partial, keyboard + mouse): signal analysis (find the interference, align two channels, read the source from arrival times), circuit diagnosis (three measurements, name the module, pick a bypass that carries the load), evidence timeline (order, find the boundary, say what it means), route planning (round trip within the oxygen budget, sealed corridor, optional scan). Seeded layouts; sample-pair picking and fixed lever answers removed. Gate `minigame_100_tests.gd`.
- **UI**: logs follow new lines only while you are at the bottom (a "새 발언 ↓" chip otherwise), scroll after layout settles, keep room under the last line; wrapped choices grow with their text; shared spacing tokens; no clipped quotes. Gate `ui_layout_100.gd` at 1366×768, 1600×900, 1920×1080, 2560×1440.
- Duplicate cross-character lines rewritten per voice; reply context no longer shows an empty name; "자기 선실" wording; research record `docs/RESEARCH_100.md`; human-readable `walkthrough_100.gd`.

## 0.9.0 — HUMAN VARIABLE

- **Explorers.** A new campaign starts by choosing one of six explorers (세린·미카·제이스·라엘·로건·시아), not a look. The selection screen shows the owner's illustrations (card bust, full figure), the explorer's own pixel self walking and greeting, background, reason for joining and how they approach people — no stats or difficulty marks. `explorer_id` and `art_id` are stored separately; legacy p1–p6 saves stay neutral explorers with their name and look.
- **Art mapping fixed.** An earlier draft used generated portraits and mapped the explorers to the temporary p1–p6 sheets. All six now use the owner's drawings: `플레이어/<이름>.png` (full figure), the four 3×2 expression sheets (busts + head, `tools/import_explorers_090.gd`) and `미니 도트 캐릭/<이름>.png`, `<이름>1.png`, `<이름>2.png` (walk, walk with tool, poses). The explorer's face next to their lines in conversation and meeting is the illustrated head.
- **Identity in play.** Different question wording; a short first-contact exchange with each crew member (tone tags such as intellectual_resonance, protective_friction, quiet_trust) rotating by Stage; each explorer's own wording for every meeting move. Truth, Null assignment, clues, weights and votes are identical for the same seed (tested).
- **Pixel motion.** New importer `tools/import_pixel_090.gd` (replaces import_pixel_080) cuts every walking sheet, the explorers' 4-frame walks and tool walks, and 176 drawn pose cells from the owner's action sheets for all 14 people, each scaled to the walking figure's standing size (height + head width) and anchored at the feet, with effects outside the body dropped. Frames widened to 128×128 so long hair and coats are not clipped (0 clipped frames). Two crew action sheets filed under each other's name (루칸1 = Soren, 소렌2 = Lucan) are mapped by drawing. Mirrored side rows fixed for three explorer sheets.
- **Transitions.** Drawn poses (greet, talk, listen, think, happy, surprised, cheer, sigh, work, crouch, sit, point, ready, carry) play with a one-pixel dip in and out; walks start with a push-off and stop with a settle; turning round in place passes through a side/front frame; talking characters gesture every few seconds; listeners sometimes listen/think; approaching crew greet by temperament; task success/partial reactions use drawn cheer/slump. Reduce motion keeps poses but drops dips and hops.
- Meeting arguments keep their source, player response and closure in snapshot-safe Stage state. Three public-source checks per Day are separate from the decisive intervention (and EMPATH); spending the strong intervention no longer auto-skips the remaining windows. Final statements recall checks and defenses. Contextual resistance uses shared innocent/Null vocabulary.
- Signal tuning checks frequency and phase; power routing varies the safe bypass; log ordering asks for an interpretation; new specimen comparison and round-trip route-budget tasks.
- Snapshot v4 and meta save v12 unchanged (optional nested fields).
