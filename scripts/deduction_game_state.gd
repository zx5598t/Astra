...
func submit_theory(primary_id: String, secondary_id: String, confidence: int) -> Dictionary:
    ...
    active_theory = {"day":day,"primary_id":primary_id,"secondary_id":secondary_id,"confidence":clampi(confidence,0,100),"support":support,"questions":questions,"contradictions":contradictions}
...
func theory_summary() -> String:
    if active_theory.is_empty(): return "아직 제출한 가설이 없다."
...
func _theory_support_count(suspects: Array) -> int:
    ...
    for link in manual_links:
        if str(link.get("kind","")) == "suspect" and str(link.get("target_id","")) in suspects: count += 1
    return count

func _theory_question_count(suspects: Array) -> int:
    var count: int = 0
    for link in manual_links:
        if str(link.get("kind","")) == "question" and str(link.get("target_id","")) in suspects: count += 1
    return count

func _theory_contradiction_count(suspects: Array) -> int:
    var count: int = 0
    for item in contradiction_register:
        if str(item.get("npc_id","")) in suspects: count += 1
    return count

func vote(target_id: String) -> String:
    if not theory_ready_for_vote(): return "먼저 이번 Day의 CASE THEORY를 제출해야 한다."
...
func _grade_final_theory() -> void:
    ...
    var matched: int = 0
    for npc_id in picks:
        if str(npc_id) in hidden_null_ids: matched += 1
    ...
    if matched == 2 and confidence >= 65: calibration_bonus = 10
    elif matched == 0 and confidence >= 80: calibration_bonus = -10
    elif matched == 1 and confidence >= 45 and confidence <= 80: calibration_bonus = 5
    var grade: int = clampi(identity_score + evidence_score + calibration_bonus, 0, 100)
    var label: String = "FRAGMENTED"
    if grade >= 90: label = "S-RANK OBSERVATION"
    elif grade >= 75: label = "A-RANK THEORY"
    elif grade >= 55: label = "B-RANK THEORY"
    elif grade >= 35: label = "C-RANK THEORY"
...
func final_theory_bbcode() -> String:
    if final_theory_result.is_empty(): return ""
    ...
    for npc_id in hidden_null_ids: null_names.append(str(npcs[npc_id].display_name) if npc_id in npcs else str(npc_id))
...
func _personal_event_template(npc_id: String) -> Dictionary:
    ...
    if event.is_empty(): return event
    var npc = npcs[npc_id]
