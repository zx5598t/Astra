...
func _launch_case(preferred: String = "") -> void:
    ...
    _stage_case_key = ""
    game = AstraDeductionGameState.new()
    var deduction := game as AstraDeductionGameState
    deduction.preferred_case_id = preferred
...
func _deduction_game() -> AstraDeductionGameState:
    return game as AstraDeductionGameState
...
func _refresh_actions() -> void:
    ...
    if deduction.phase_name() == "RESULT":
        var review := deduction.final_theory_bbcode()
        if review != "":
...
func _install_incident_stage(deduction: AstraDeductionGameState) -> void:
    var key := "%d:%s" % [deduction.seed_value, deduction.case_id]
    if _stage_case_key == key:
        return
    _stage_case_key = key
...
func _show_theory_vote_gate(deduction: AstraDeductionGameState) -> void:
    ...
    if deduction.theory_ready_for_vote():
        ...
        for npc_id in deduction.crew_order:
            var npc: NPCState = deduction.npcs[npc_id]
            if not npc.alive:
                continue
            _add_action("%s · %s 격리" % [npc.display_name, npc.job], c_red, _vote.bind(npc_id), "이번 Day의 격리표")
...
func _show_personal_event(hypothesis: AstraHypothesisGameState) -> void:
    ...
    if npc_id in deduction.npcs:
        npc_name = deduction.npcs[npc_id].display_name
    ...
    for i in range(choices.size()):
        ...
        var accent := c_green if i == 0 else c_gold if i == 2 else c_panel
...
func _record_theory_meta_if_needed(deduction: AstraDeductionGameState) -> void:
...
func _refresh() -> void:
    ...
    if deduction == null:
        return
    if phase_label != null:
        var theory_tag := "THEORY ✓" if deduction.theory_ready_for_vote() else "THEORY —"
