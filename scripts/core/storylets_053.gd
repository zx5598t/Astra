...
const CREW_PACKS := {
    "rho":[
        ...
        ["RELATIONSHIP","pair","PAIR","rho_sena_defense_callback","준은 세나가 지난 회의에서 자기 편을 들었던 말을 농담으로 넘기지 않는다.","그때 말해 준 건… 기억해.",{"requires":{"memory_tag":"sena_defended_rho"},"rarity":"uncommon"}],
    ...
    "sena":[
        ...
        ["RELATIONSHIP","pair","PAIR","sena_rho_defense_memory","세나는 준에게 별말 없이 출입 권한을 먼저 열어 준다.","어제 네 말은 들었어. 오늘은 작업부터 해.",{"requires":{"memory_tag":"rho_defended_sena"},"rarity":"uncommon"}],
    ...
    "vale":[
        ...
        ["RELATIONSHIP","pair","PAIR","vale_eli_quiet_callback","소렌은 루칸이 준 좌표만 적고 그 이유는 묻지 않는다.","설명은 나중에 들어도 돼요. 위치는 지금 필요하니까.",{}],
    ...
    "eli":[
        ...
        ["RELATIONSHIP","pair","PAIR","eli_vale_callback","루칸은 소렌의 신호 시각을 들은 뒤 말없이 지도 한 점을 확대한다.","여기면 설명 하나는 줄어.",{}],
...
const SOCIAL_SCENES := [
    {"id":"053_mira_lyra_oxygen","speaker":"mira","target":"lyra","tag":"pair","category":"PAIR","family":"mira_lyra_resources","intent":"conflict","action":"의료실 산소 배분표와 생태실 환기표가 같은 자원을 요구한다.","lines":[["lyra","이쪽을 더 줄이면 내일 공기 질이 떨어져요."],["mira","알아요. 오늘 밤을 넘길 사람도 있어요."],["lyra","그럼 두 시간만. 그 뒤엔 다시 돌려요."],["mira","좋아요. 제가 시간 적을게요."]],"line_relations":["anchor","challenge","proposal","agreement"],"chapters":MID_CHAPTERS},
    ...
    {"id":"053_mira_sena_bandage","speaker":"mira","target":"sena","tag":"pair","category":"DAILY","family":"mira_sena_bandage","intent":"humor","action":"미라는 세나가 또 가져가려는 붕대 상자를 한 칸 위로 옮긴다.","lines":[["sena","거기 둬도 닿아."],["mira","알아요. 다음엔 영수증 끊어 드리려고요."],["sena","그건 됐어."]],"line_relations":["anchor","tease","reply"],"chapters":MID_CHAPTERS},
    ...
    {"id":"053_mira_rho_quiet","speaker":"mira","target":"rho","tag":"pair","category":"RELATIONSHIP","family":"mira_rho_quiet","intent":"care","action":"준의 농담이 멈춘 걸 본 미라는 상처 대신 먼저 경보 기록을 닫는다.","lines":[["mira","오늘은 안 웃네요."],["rho","웃을 타이밍이 아니잖아."],["mira","네. 그래서 물어본 거예요."]],"line_relations":["anchor","reply","clarify"],"chapters":MID_CHAPTERS,"rarity":"uncommon"},
    ...
    {"id":"053_mira_noa_consent","speaker":"mira","target":"noa","tag":"pair","category":"RELATIONSHIP","family":"mira_noa_consent","intent":"work","action":"미라는 환자 이름을 가린 사본을 노아에게 건넨다.","lines":[["mira","이 정도면 시간은 확인할 수 있어요."],["noa","이름은 없어도 돼요. 지금 필요한 건 순서니까."],["mira","그럼 원본은 제가 보관할게요."]],"line_relations":["proposal","accept","resolution"],"chapters":MID_CHAPTERS},
    ...
    {"id":"053_mira_dax_body","speaker":"mira","target":"dax","tag":"pair","category":"MYSTERY","family":"mira_dax_body","intent":"mystery","action":"미라는 19년 전 기록 옆에 현재 신체 수치를 놓고 다렌에게 화면을 돌린다.","lines":[["mira","기록이 맞다면 몸이 안 맞아요."],["dax","몸이 맞다면 기록이 안 맞고."],["mira","둘 다 맞을 가능성은요?"],["dax","그걸 계산하려면 가정부터 바꿔야 해."]],"line_relations":["anchor","support","question","reply"],"chapters":["SILENT_ORBIT","LAST_LIGHT"]},
...
static func _scene_from_pack(npc_id: String, index: int, pack: Array, prefix: String) -> Dictionary:
    ...
    for key in ["requires","forbids","rarity","deviation_reason","source_event","possible_followup",
        "player_specific","agency","tone_lines","tone_actions","role_lines","knowledge_share","promise"]:
        if meta.has(key):
            scene[key] = meta[key]
    return scene
