...
const FORE_PACKS := {
    "mira":["미라는 경보가 울리기 전 당신이 의료함을 연 것을 본다.","아직 요청도 없었는데 준비했네요. 이유는 나중에라도 말해 줘요."],
    "rho":["준은 릴레이 커버를 당신이 먼저 연 것을 본다.","고장 나기 전에 그걸 왜 먼저 열었어?"],
...
const INCIDENT_AFTER := {
    ...
    "dax":["OXYGEN_BALANCE","다렌은 의료와 생태 양쪽 산소 요구량을 같은 표에 남긴다.","어느 쪽이 틀린 게 아니야. 같은 여유분을 두 기준으로 본 거야."],
