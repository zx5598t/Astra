...
const TOOLTIPS = {
    "investigate": "현장에서 기록이나 흔적을 찾습니다. 30분이 흐릅니다.",
...
const SCREEN_HELP = {
    ...
    "INVESTIGATION": {
        ...
        "points": [
            "장소 이동은 무료입니다. 조사 지점을 누를 때만 30분이 흐릅니다.",
    ...
    "RESULT": {
        ...
        "points": [
            ...
            "졌더라도 같은 사건을 다시 재구성할 수 있습니다. 배치는 새로 결정됩니다."
        ]
    }
}
...
const TOPICS = [
    ...
    {"id": "clue", "needs": "", "title": "흔적은 이름이 아니다", "body": "현장 흔적은 ‘내열 섬유 — Rho, Dax’처럼 후보 명단을 줍니다. 한 명을 특정하려면 같은 조작에서 나온 흔적 두 개의 명단을 겹쳐야 합니다. 기록 시각이 사건 시간대 밖이면 그 흔적은 사건과 무관합니다."},
    ...
    {"id": "meeting", "needs": "", "title": "회의", "body": "공개된 근거만 여론을 움직입니다. 확보했지만 공개하지 않은 단서는 다른 승무원의 판단에 영향을 주지 않습니다. 근거 없는 지목은 조사관의 신뢰를 떨어뜨립니다."},
    {"id": "vote", "needs": "", "title": "투표", "body": "조사관의 표는 2표로 계산됩니다. 최다 득표자를 격리하며, 동률에 조사관의 대상이 있으면 그 대상을 우선합니다."},
    ...
    {"id": "loop", "needs": "case_select", "title": "왜 같은 사람들이 다시 나오는가", "body": "ASTRA가 복원하는 것은 과거의 재생이 아니라, 손상된 기록이 허용하는 사건 모델입니다. 물리적 사실은 고정되어 있지만 신원 바인딩이 훼손돼 누가 어느 역할이었는지는 재구성마다 다시 계산됩니다. 그래서 이전 사건의 역할은 다음 사건의 증거가 될 수 없습니다."}
]
