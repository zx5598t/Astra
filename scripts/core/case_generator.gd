...
static func _build_day_packet(case_id: String, stage_seed: int, day: int, active: Array, living_nulls: Array, difficulty: String, heat: Dictionary, attempt: int) -> Dictionary:
    ...
    var deduction_profile: Dictionary = data.get("deduction_profile", {})
    var specific_scale := float(deduction_profile.get("specific_scale", 1.0))
    ...
    var second_witness := ""
    var second_group := {}
    var force_two_source := bool(deduction_profile.get("force_two_source", false))
    if not w_specific and not e_specific and ((best_cost >= 1.0 and want >= 2) or force_two_source):
        # Intersection profiles prefer two partial sources whose overlap does
        # the narrowing. The generator reads the authored topology, not a case
        # name, so another Stage can opt into the same deduction shape.
