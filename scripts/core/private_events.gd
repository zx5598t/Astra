...
const EVENTS := {
    "mira": {
        "scene": "의료실 비상등이 낮게 깜빡인다. Mira는 장갑을 벗지 않은 채 자기 손끝만 내려다본다.",
    ...
    "rho": {
        "scene": "엔진 진동이 바닥을 울린다. Rho가 렌치를 작업대에 내려놓고 당신을 똑바로 본다.",
    ...
    "eli": {
        "scene": "항법창 너머로 별빛이 느리게 흐른다. Eli가 동전을 튕기다가 손바닥으로 탁 덮는다.",
        ...
        "choices": [
            ...
            {"label": "“그럼 넌 이번 판을 어떻게 읽고 있지?”", "hint": "Eli의 속마음을 듣는다", "effect": "top_suspect"}
        ]
    },
    "sena": {
        "scene": "보안허브의 감시 화면들이 일제히 당신을 비춘다. Sena가 출입문을 잠그고 돌아선다.",
        "prompt": "“절차대로 가면 범인을 놓칠 수 있습니다. 오늘 밤 제 재량으로 선내 순찰을 강화해도 되겠습니까?”",
        "choices": [
            {"label": "“허가합니다. 오늘 밤 순찰을 맡기죠.”", "hint": "오늘 밤 Sena가 한 명을 추가로 보호한다", "effect": "patrol"},
            {"label": "“아니요. 절차를 지킵시다.”", "hint": "신뢰 상승 · 긴장 완화", "effect": "procedure"},
            {"label": "“순찰 대상은 누구로 생각하고 있죠?”", "hint": "Sena가 주시하는 사람을 듣는다", "effect": "top_suspect"}
        ]
    },
    "vale": {
        "scene": "통신실 스피커에서 백색소음이 흐른다. Vale이 채널 하나를 손수 끄고 목소리를 낮춘다.",
        "prompt": "“조사관님께만 먼저 보여 드릴게요. 그날 밖으로 나간 신호, 해석이 하나만은 아니거든요.”",
        "choices": [
            {"label": "“보여 주세요.”", "hint": "Vale이 해석한 기록을 받는다 · 출처는 Vale", "effect": "vale_record"},
            {"label": "“공식 기록으로만 받겠습니다.”", "hint": "신뢰 소폭 하락", "effect": "cold"},
            {"label": "“왜 하필 저에게만 보여 주는 거죠?”", "hint": "반응을 관찰한다", "effect": "read"}
        ]
    },
    "noa": {
        "scene": "기록보관실의 로그가 끝없이 흘러간다. Noa가 문장 하나에서 스크롤을 멈춘다.",
    ...
    "lyra": {
        "scene": "수목구역 유리벽에 습기가 맺힌다. Lyra가 시든 잎 하나를 조심스럽게 접는다.",
    ...
    "dax": {
        "scene": "진단 패널의 그래프가 규칙적으로 뛴다. Dax가 수치 하나를 손가락으로 짚는다.",
        "prompt": "“사람의 직감과 시스템 로그가 충돌하면, 무엇을 우선하지?”",
        "choices": [
            {"label": "“로그.”", "hint": "Dax가 추리의 핵심 변수를 짚어 준다", "effect": "dax_hint"},
            {"label": "“직감.”", "hint": "신뢰 소폭 하락", "effect": "cold"},
            {"label": "“둘 다 틀릴 수 있어.”", "hint": "Dax의 속마음을 듣는다", "effect": "top_suspect"}
        ]
    }
}
