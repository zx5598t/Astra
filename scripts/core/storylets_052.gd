...
const CHARACTER_PACKS := {
    "rho": [
        ...
        ["personal","PERSONAL","daren_respect","준이 다렌의 계산표를 구겨 버리려다 펴서 다시 놓는다.","짜증 나는 건 맞는데, 저 계산 없으면 나도 못 고쳐."],
        ...
        ["personal","RELATIONSHIP","player_tool","준이 당신이 주워 둔 작은 드라이버를 알아본다.","그거 아직 갖고 있었네. 내가 잃어버린 줄 알았는데.",{"choices":[{"label":"드라이버를 건넨다.","effect":"help"},{"label":"“나중에 필요할 것 같아서.”","effect":"record"}]}],
        ["conflict","CONFLICT","daren_method","준이 다렌의 ‘정상 범위’ 표시 위에 진동 파형을 겹친다.","정상 범위가 정상 소리를 보장하진 않아."],
    ...
    "lyra": [
        ...
        ["personal","RELATIONSHIP","daren_numbers","마렌이 다렌의 표에서 수치 대신 샘플 이름에 밑줄을 긋는다.","숫자는 다렌이 기억할 거예요. 저는 이게 뭐였는지 기억할게요."],
        ...
        ["echo","ECHO","familiar_soil","마렌이 처음 보는 흙 봉투를 열기 전에 냄새부터 맡고 멈춘다.","이 냄새… 처음은 아닌 것 같아요.",{"requires":{"min_loop":1},"rarity":"rare","deviation_reason":"MEMORY_MISMATCH","source_event":"old_sample_echo","possible_followup":"compare_soil_origin"}],
    ...
    "noa": [
        ...
        ["personal","RELATIONSHIP","daren_order","노아가 다렌의 원인순 정렬과 자신의 시간순 정렬을 둘 다 남긴다.","어느 쪽이 맞는지보다 둘이 왜 다르게 보이는지가 먼저예요."],
    ...
    "mira": [
        ...
        ["everyday","DAILY","warm_room","미라가 의료실 온도를 0.5도 올리고 사람들 표정을 살핀다.","같은 온도도 수면 뒤엔 더 춥게 느껴져요."],
        ...
        ["echo","ECHO","familiar_bandage","미라가 처음 보는 상처 위치를 보고 이미 붕대를 꺼내 든 자신을 알아차린다.","왜 여기 다칠 거라고 생각했을까요?",{"requires":{"min_loop":1},"rarity":"rare","deviation_reason":"PROTECTION_ECHO","source_event":"prior_protection","possible_followup":"mira_protects_same_person"}],
    ...
    "eli": [
        ...
        ["echo","ECHO","known_turn","루칸이 처음 본 항로의 다음 회전을 화면보다 먼저 손으로 짚는다.","…왜 다음이 여기라고 알고 있었지?",{"requires":{"min_loop":1},"rarity":"rare","deviation_reason":"MEMORY_MISMATCH","source_event":"route_echo","possible_followup":"eli_route_compare"}],
...
const SOCIAL_SCENES := [
    ...
    {"id":"052_pair_mira_lyra_rest","speaker":"lyra","target":"mira","tag":"pair","category":"PAIR","family":"mira_lyra_care","intent":"relationship","action":"마렌이 의료실 창가에 작은 화분을 놓고 미라의 손을 본다.","lines":[["lyra","화분은 제가 챙길게요. 미라는 잠을 챙겨요."],["mira","그 말, 제가 더 자주 하는 말인데요."]],"choices":[],"chapters":MID_CHAPTERS},
    ...
    {"id":"052_pair_sena_mira_door","speaker":"sena","target":"mira","tag":"pair","category":"PAIR","family":"sena_mira_protect","intent":"relationship","action":"세나가 의료실 문을 절반만 열어 두고 복도를 확인한다.","lines":[["mira","그냥 닫아도 돼요."],["sena","안 돼. 안에서 무슨 일 생기면 바로 들어와야 해."]],"choices":[],"chapters":MID_CHAPTERS},
    {"id":"052_pair_rho_dax_sound","speaker":"rho","target":"dax","tag":"pair","category":"PAIR","family":"rho_dax_respect","intent":"conflict","action":"준이 다렌의 정상 판정 옆에 녹음기를 올려놓는다.","lines":[["rho","숫자로는 정상이지?"],["dax","평균은."],["rho","그럼 평균 말고 이 소리도 들어."]],"choices":[],"chapters":MID_CHAPTERS},
