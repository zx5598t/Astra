...
func _initialize() -> void:
    ...
    check(meta.campaign_complete(), "all chapters complete")
    check(meta.story_archive().size() == AstraCaseCatalog.CAMPAIGN.size(), "one archive entry per campaign case")
    check(meta.save_data(), "archive saved")
    var roundtrip := AstraMetaProgress.new(PATH)
    roundtrip.load_data()
    check(roundtrip.mission_badges.size() == AstraCaseCatalog.CAMPAIGN.size() and roundtrip.campaign_complete(), "archive progress persists")
    AstraGameSession.delete_snapshot(PATH)
