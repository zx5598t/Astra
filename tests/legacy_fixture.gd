...
func _initialize() -> void:
    ...
    s.advance()
    if not s.pending_event.is_empty():s.resolve_private_event(mini(1,s.pending_event.get("choices",[]).size()-1))
    ...
    var saved_rng := s.rng.state
    var saved_clues := s.clues.duplicate(true)
    s.cast_vote(str(s.living_ids()[0]))
    s.advance()
    if s.phase=="NIGHT":s.choose_night_action("protect",str(s.living_ids()[0]))
    var cfg := ConfigFile.new()
    cfg.set_value("expected","saved_rng",saved_rng)
    cfg.set_value("expected","saved_clues",saved_clues)
